...
export const CartPanel = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`scroll overflow-x-hidden max-h-[400px] w-full overflow-y-scroll py-3 px-4 mb-12  ${
          cartData?.length === 0 ? "" : "shadow-md"
        }`}
      >
        {cartData?.length !== 0 ? (
          cartData?.map((item) => (
          ...
          <div className="w-full h-[300px] flex items-center justify-center text-xl font-bold text-black/40 gap-3">
            <p className="font-['lexend']">Your Cart Is Empty</p>
            <TbShoppingBagExclamation className="text-2xl" />
          </div>
        )}
      </div>
      {cartData?.length !== 0 && (
